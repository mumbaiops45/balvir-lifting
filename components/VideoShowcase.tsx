    ...
    <section id="videos" ref={sectionRef} className="section-pad bg-gray-50 relative overflow-hidden ">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div ref={headRef} className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <p className="text-blue-600 text-xs font-bold uppercase tracking-[0.25em] mb-4">Video Showcase</p>
            <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 leading-tight tracking-tight">
              See the World We <span className="text-blue-gradient">Build</span>
            </h2>
          </div>
          ...
          {showcaseVideos.map(v => (
            <button key={v.id} onClick={() => setFeatured(v)}
              className={`group relative overflow-hidden aspect-video bg-gray-200 border transition-all duration-300 ${
                featured.id === v.id
                  ? "border-blue-600 shadow-[0_0_20px_rgba(26,79,196,0.25)]"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2">
                <span className="text-[9px] font-bold uppercase tracking-wider text-blue-300 block">{v.category}</span>
  ...
  return (
    <div id="featured-player"
      ...
      <div className="absolute top-4 left-4 pointer-events-none">
        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white bg-blue-600 px-3 py-1.5">
          {video.category}
        </span>
      </div>
